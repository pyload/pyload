...
class AbstractDictionaryNSManager(NamespaceManager):
    ...
    def get_creation_lock(self, key):
        return NameLock(
            identifier="memorynamespace/funclock/{}/{}".format(self.namespace, key),
            reentrant=True
        )
...
class DBMNamespaceManager(OpenResourceNamespaceManager):
    ...
    def get_creation_lock(self, key):
        return file_synchronizer(
                    identifier = "dbmcontainer/funclock/{}".format(self.namespace),
                    lock_dir=self.lock_dir
                )
    ...
    def do_open(self, flags):
        debug("opening dbm file {}", self.file)
        try:
    ...
    def do_close(self):
        if self.dbm is not None:
            debug("closing dbm file {}", self.file)
            self.dbm.close()
...
class FileNamespaceManager(OpenResourceNamespaceManager):
    ...
    def get_creation_lock(self, key):
        return file_synchronizer(
                identifier = "filecontainer/funclock/{}".format(self.namespace,)
                lock_dir = self.lock_dir
                )
