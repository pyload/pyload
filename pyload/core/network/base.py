...
class Base(object):
    """The Base plugin class with all shared methods and every possible
    attribute for plugin definition."""
    ...
    def get_storage(self, key=None, default=None):
        """Retrieves saved value or dict of all saved entries if key is
        None."""
        if key is not None:
    ...
    def check_abort(self):
        """Will be overwritten to determine if control flow should be
        aborted."""
        if self.abort():
            raise Abort
