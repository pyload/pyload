...
class PluginLoader(object):
    ...
    def parse_plugin(self, filename, folder, name):
        """Parses a plugin from disk, folder means plugin type in this context.
        Also sets config.
    ...
    def iter_plugins(self):
        """Iterates over all plugins returning (type, name, info)  with info as
        PluginTuple."""
    ...
    def remove_plugin(self, type_, name, version=None):
        ...
        Optionally only when its version is below or equal the available
        one
    ...
    def is_user_plugin(self, name):
        """Determine if given plugin name is enable for user_context in any
        plugin type."""
