...
class SerienjunkiesOrg(Crypter):
    ...
    def handle_oldStyleLink(self, url):
    ...
    def decrypt(self, pyfile):
        ...
        if framePattern.match(url):
            self.packages.append((self.pyfile.package().name, [self.handle_frame(url)], self.pyfile.package().name))
        elif episodePattern.match(url):
            self.handle_episode(url)
        elif oldStyleLink.match(url):
            self.handle_oldStyleLink(url)
        elif showPattern.match(url):
            self.handle_show(url)
        elif showPatternDJ.match(url):
            self.handle_showDJ(url)
        elif seasonPattern.match(url):
            self.handle_season(url)
        elif categoryPatternDJ.match(url):
            self.handle_categoryDJ(url)
