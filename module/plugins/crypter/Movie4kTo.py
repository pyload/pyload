...
class Movie4kTo(Crypter):
    ...
    DETECT_JS_LINK_PATTERN = r'links\[(\d+?)\].+&nbsp;(.+?)</a>(.+?)</tr>'
    DETECT_HTML_LINK_PATTERN = r'<tr id="tablemoviesindex2".*?<a href=".*?(\d{4,7}).*?">.*?&nbsp;(.+?)</a>(.+?)</tr>'
    ...
    def handle_season_from_episode(self, url_path):
        ...
        self.logDebug('finding dropdown pregrep pattern: %s' % pregrep_pattern)
        selected_episodes = re.search(pregrep_pattern, self.html, re.MULTILINE | re.DOTALL)

        self.logDebug('finding %s in %s' % (self.EPISODE_DROPDOWN_PATTERN, selected_episodes.group(0)))
    ...
    def handle_other(self, url_path):
        ...
        if pattern_re is not None:
            if pattern_re.group('mode') == 'season':
                ...
                self.logDebug('handling TV show ' + name)
                ...
                self.logDebug('handling season ' + seasonNR + ' of ' + name)
    ...
    def decrypt(self, pyfile):
        ...
        if (self.format == 'movie') or (self.format == 'episode'):
            if(self.getConfig('whole_season') and (self.format == 'episode')):
    ...
    def getLinks(self, url_path):
        ...
        self.logDebug('found the following matches in %s/%s:' % (self.BASE_URL, url_path))
        ...
        for h_id, hoster, q_html in matches:
            if hoster not in hoster_blacklist:
                ...
                if match_q:
                    ...
                    if self.max_q == None:
                        ...
                        if self.max_q < quality:
                            self.max_q = quality
                    self.logDebug('Quality: %d' % quality)
                ...
                if count[hoster] <= firstN:
                    ...
                    if h_id != self.id:
                        if self.format != 'movie':
                            self.logDebug('detected TV show episode, loading %s/tvshows-%s-%s.html' % (self.BASE_URL, h_id, self.name))
                            self.html = self.load('%s/tvshows-%s-%s.html' % (self.BASE_URL, h_id, self.name))
                        else:
                            self.logDebug('detected movie, loading %s/%s-watch-movie-%s.html' % (self.BASE_URL, self.name, h_id))
                ...
                self.logDebug('Not accepted since %s is blacklisted' % hoster)
        # self.logDebug(links)
        return links
