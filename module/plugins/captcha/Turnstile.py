...
class Turnstile(CaptchaService):
    ...
    def detect_key(self, data=None):
        ...
        if m is not None:
            ...
            if m is not None:
                ...
                self.log_debug(key, "Wrong key format, this probably because is is not a Turnstile key")
    ...
    def _challenge_js(self, key):
        ...
        params = {'url': self.pyfile.url,
                  'sitekey': key,
                  'script': {'signature': self.TURNSTILE_INTERACTIVE_SIG,
...
if __name__ == "__main__":
    # Sign with the command `python -m module.plugins.captcha.Turnstile pyload.private.pem pem_passphrase`
    import sys
    ...
    if len(sys.argv) > 2:
        ...
        print sign_string(Turnstile.TURNSTILE_INTERACTIVE_JS, pem_private, pem_passphrase=sys.argv[2], sign_algo="SHA384")
