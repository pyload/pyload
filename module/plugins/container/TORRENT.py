...
def get_info_hash(input_data):
    ...
    """
    def _get_info_section_from_torrent(torrent_content):
        """
    ...
    def _extract_info_hash_from_torrent(torrent_content):
        """
        ...
        try:
            ...
            info_hash = digest.finalize()
            return info_hash.encode('hex')
        except Exception as e:
            raise ValueError("Failed to extract info hash: %s" % e)

    def _extract_info_hash_from_magnet(magnet_url):
        """
        ...
        BTV2_HASH_KEY = "urn:btmh:"

        up = urlparse.urlparse(magnet_url)
            ...
            for xt in xts:
                if xt.startswith(BTV1_HASH_KEY):
                    ...
                    if len(xt) == 40:
                        ...
                        info_hash = base64.b32decode(xt).encode('hex')
                        break
    ...
    if isinstance(input_data, str):
        up = urlparse.urlparse(input_data)
        # Handle magnet URL
        if up.scheme == "magnet":
    ...
    elif hasattr(input_data, 'read') and callable(getattr(input_data, 'read')):
