...
class AlldebridCom(MultiHoster):
    ...
    __config__ = [("activated", "bool", "Activated", True),
                  ...
                  ("stream_quality", "Lowest;LD 144p;LD 240p;SD 380p;HQ 480p;HD 720p;HD 1080p;Highest", "Quality to download from stream hosters", "Highest")]
    ...
    def handle_premium(self, pyfile):
        ...
        if api_data.get("error", False):
            ...
            if api_data['link'] == "" and "streams" in api_data:
                ...
                streams = dict([
                    (_s['quality'], {'ext': _s['ext'], 'filesize': _s['filesize'], 'id': _s['id']})
                    for _s in api_data['streams']
                    if type(_s['quality']) == int])
                qualities = sorted(streams.keys())
                self.log_debug("AVAILABLE STREAMS: %s" % qualities)
                ...
                self.log_debug("CHOSEN STREAM: %s" % chosen_quality)
                ...
                api_data = self.api_response("link/streaming",
                                             get={'apikey': self.account.info['login']['password'],
                                                  'id': unlock_id,
                                                  'stream': stream_id})
                if api_data.get("error", False):
                    self.log_error(api_data['error']['message'])
                ...
                if delayed_id:
                    pyfile.setCustomStatus("delayed stream")
                    while True:
                        api_data = self.api_response("link/delayed",
                                                     get={'apikey': self.account.info['login']['password'],
                                                          'id': delayed_id})
                        if 'link' in api_data:
