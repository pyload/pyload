...
class Keep2ShareCc(SimpleHoster):
    ...
    def seconds_until_midnight_utc():
        ...
        midnight_utc = datetime.utcnow().replace(
    ...
    def check_errors(self, data=None):
        if data is not None:
            ...
            if json_data.get("errorCode"):
                err = json_data["errorCode"]
                self.log_debug(_("Handling error code: %s") % err)

                if err in [Keep2ShareCc.ErrorCode.CAPTCHA_REQUIRED, Keep2ShareCc.ErrorCode.CAPTCHA_INVALID]:
                    self.retry_captcha()

                elif err == Keep2ShareCc.ErrorCode.DOWNLOAD_NOT_AVAILABLE:
                    self.retry(wait=json_data["errors"][0]["timeRemaining"])

                elif err == Keep2ShareCc.ErrorCode.FILE_IS_NOT_AVAILABLE:
