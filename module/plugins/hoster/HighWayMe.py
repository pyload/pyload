...
class HighWayMe(MultiHoster):
    ...
    def handle_premium(self, pyfile):
        for _i in range(5):
            ...
            self.log_warning(_("Unable to get API data, waiting 1 minute and retry"))
        ...
        try:
            ...
            self.log_warning(_("Name pattern not found"))

        try:
            self.pyfile.size = re.search(r'<size>(\d+)</size>', self.data).group(1)

        except AttributeError:
            self.log_warning(_("Size pattern not found"))
            self.pyfile.size = 0
