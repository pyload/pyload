...
class WetransferCom(SimpleHoster):
    ...
    __description__ = """Wetransfer.com hoster plugin"""
    __license__ = "GPLv3"
    __authors__ = [("GammaC0de", "nitzo2001[AT]yahoo[DOT]com")]

    URL_REPLACEMENTS = [("https?://(?:www\.)?wetransfer.com/", "https://wetransfer.com/")]
    DIRECT_LINK = False
    ...
    def api_request(self, method, file_id, **kwargs):
        ...
        try:
            json_data = self.load("%s%s/%s/%s" % (self.API_URL, "transfers", file_id, method),
    ...
    def api_info(self, url):
        ...
        if recipient_id is not None:
            api_data = self.api_request("prepare-download",
                                        file_id,
    ...
    def handle_free(self, pyfile):
        ...
        if recipient_id is not None:
            api_data = self.api_request("download",
                                        file_id,
