...
def error_description(error_code):
    err_message = {'notLink': "Check the 'link' parameter (Empty or bad)",
                   ...
                   'maxTorrent': "Limitation of torrents per day reached"}.get(error_code)

    return err_message or "Unknown error: '%s'" % error_code
...
class DebridlinkFr(MultiHoster):
    ...
    def handle_premium(self, pyfile):
        api_data = self.api_request("v2/downloader/add",
                                    post={'url':pyfile.url})
