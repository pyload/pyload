...
class DownloadControl(Hook):
    __name__ = 'DownloadControl'
    __version__ = '0.01'
    __description__ = 'Tune chunks and parallels download settings with rules'
    __config__ = [
    ...
    event_map = {'downloadStarts': 'downloadControl'}
    ...
    def syncSetting(self, coresync=False):
        ...
        last = self.info['last']
        if now != last:
            msg = 'detected core chunks setting was changed: store newer'
            self.logDebug(msg)
            self.info['orig'] = now
        if coresync:
            self.config['download']['chunks'] = self.info['orig']

    def downloadControl(self, pyfile, url, filename):
        premium = pyfile.plugin.premium
        hosters = self.getConfig('hosters')
        if hosters:
            hosters = hosters.replace('|', ',').replace(';', ',').split(',')
            msg = 'overwrited default rules with %s'
            self.logDebug(msg % hosters)
        self.syncSetting()
