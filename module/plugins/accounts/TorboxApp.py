...
from ..internal.misc import json, reduce
    ...
    def signin(self, user, password, data):
        ...
        if api_data.get("success", False):
            if api_data["data"]["email"] != user:
                self.log_error(_("Username for TorboxApp should be your torbox.app email"))
                self.fail_login()

        else:
            self.log_error(_("Password for TorboxApp should be your torbox.app API token"))
            self.log_error(api_data["detail"])
            self.fail_login()
