...
class ZeveraCom(MultiAccount):
    ...
    def init(self):
        ...
        if not hasattr(self, "API_URL"):
            self.API_URL = "http://api.%s/jDownloader.ashx" % (
                self.PLUGIN_DOMAIN or "")
    ...
    def grab_info(self, user, password, data):
        ...
        if "No trafic" not in api and api['endsubscriptiondate'] != "Expired!":
            validuntil = time.mktime(
                time.strptime(
                    api['endsubscriptiondate'],
                    "%Y/%m/%d %H:%M:%S"))
            trafficleft = float(api['availabletodaytraffic']) * \
                1024 if api['orondaytrafficlimit'] != '0' else -1
            premium = True
