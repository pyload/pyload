...
class WebServer(threading.Thread):
    ...
    def run(self):
        ...
        if self.server in ("lighttpd", "nginx"):
            log.warning(_("Sorry, we dropped support for starting {} directly within pyLoad").format(self.server))
            log.warning(_("You can use the threaded server which offers good performance and ssl,"))
            log.warning(_("of course you can still use your existing {} with pyLoads fastcgi server").format(self.server))
        ...
        if self.server == "fastcgi":
            try:
                ...
                log.warning(_("Can't use {}, python-flup is not installed!").format(self.server))
                self.server = "builtin"
    ...
    def start_builtin(self):
        ...
        self.core.log.info(_("Starting builtin webserver: {host}:{port:d}").format(**{"host": self.host, "port": self.port}))
        webinterface.run_simple(host=self.host, port=self.port)

    def start_threaded(self):
        if self.https:
            self.core.log.info(_("Starting threaded SSL webserver: {host}:{port:d}").format(**{"host": self.host, "port": self.port}))
        else:
            self.cert = ""
            self.key = ""
            self.core.log.info(_("Starting threaded webserver: {host}:{port:d}").format(**{"host": self.host, "port": self.port}))

        webinterface.run_threaded(host=self.host, port=self.port, cert=self.cert, key=self.key)

    def start_fcgi(self):

        self.core.log.info(_("Starting fastcgi server: {host}:{port:d}").format(**{"host": self.host, "port": self.port}))
    ...
    def start_lightweight(self):
        ...
        self.core.log.info(_("Starting lightweight webserver (bjoern): {host}:{port:d}").format(**{"host": self.host, "port": self.port}))
