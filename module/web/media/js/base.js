{% autoescape true %}
  ...
  humanFileSize = function(size) {
    ...
    if (size === 0) {
      ...
      return Math.round(size * 100 / a) / 100 + " " + filesizename[i];
    }
  };

  parseUri = function() {
    var oldString, part, regxp, res, resu, _i, _len;
    ...
    res = "";
    for (_i = 0, _len = resu.length; _i < _len; _i++) {
      part = resu[_i];
      if (part.indexOf(" ") !== -1) {
  ...
  document.addEvent("domready", function() {
    ...
    $('action_play').addEvent('click', function() {
      return new Request({
        method: 'get',
        url: "{{ '/api/unpauseServer'|url }}"
      }).send();
    });
    $('action_cancel').addEvent('click', function() {
      return new Request({
        method: 'get',
        url: "{{ '/api/stopAllDownloads'|url }}"
      }).send();
    });
    $('action_stop').addEvent('click', function() {
      return new Request({
        method: 'get',
        url: "{{ '/api/pauseServer'|url }}"
      }).send();
    });
  ...
  LoadJsonToContent = function(data) {
