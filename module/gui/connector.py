...
class Connector(QObject):
    ...
    def connectProxy(self):
        ...
        if not server_version == SERVER_VERSION:
            self.emit(SIGNAL("errorBox"),
                      _("server is version %(new)s client accepts version %(current)s") % {"new": server_version, "current": SERVER_VERSION})
            return False
        return True
...
class DispatchRPC(QObject):
    ...
    class Wrapper(object):
        ...
        def __call__(self, *args, **kwargs):
            ...
            except (socket.error, TException):
