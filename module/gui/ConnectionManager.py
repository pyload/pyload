...
class ConnectionManager(QWidget):
    ...
    def slotNew(self):
        data = {"id": uuid().hex, "type": "remote", "default": False, "name": "", "host": "", "port": "7228",
                "user": "admin", "password": ""}
    ...
    class EditWindow(QWidget):
        ...
        def getData(self):
            ...
            data["name"] = self.controls["name"].text()
            data["local"] = self.controls["local"].isChecked()
            data["user"] = self.controls["user"].text()
            data["password"] = self.controls["password"].text()
            data["host"] = self.controls["host"].text()
            data["port"] = self.controls["port"].value()
            data["type"] = "local" if data["local"] else "remote"
            return data
