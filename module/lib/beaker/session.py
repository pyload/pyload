...
class Session(dict):
    ...
    def _create_id(self):
        self.id = md5(
            md5("%f{}%f{}".format(time.time(), id({}), random.random(),
                              getpid())).hexdigest(),
        ).hexdigest()
        self.is_new = True
        ...
        if self.use_cookies:
            ...
            if self.cookie_expires is not True:
                if self.cookie_expires is False:
                    ...
                    raise ValueError("Invalid argument for cookie_expires: {}"
                                    .format(repr(self.cookie_expires)))
...
class CookieSession(Session):
    ...
    def _make_id(self):
        return md5(md5(
            "%f{}%f{}".format(time.time(), id({}), random.random(), getpid())
            ).hexdigest()
        ).hexdigest()
