...
var PackageUI = new Class({
    ...
    deleteFinished: function() {
        ...
        new Request.JSON({
            method: 'get',
            url: '{{pathprefix}}/api/deleteFinished',
    ...
    restartFailed: function() {
        ...
        new Request.JSON({
            method: 'get',
            url: '{{pathprefix}}/api/restartFailed',
            onSuccess: function(data) {
                this.packages.each(function(pack) {
                    pack.close();
                });
    ...
    saveSort: function(ele, copy) {
        ...
        if (order.length > 0) {
            ...
            new Request.JSON({
                method: 'get',
                url: '{{pathprefix}}/json/package_order/' + order[0],
...
var Package = new Class({
    ...
    loadLinks: function() {
        ...
        new Request.JSON({
            method: 'get',
            url: '{{pathprefix}}/json/package/' + this.id,
    ...
    createLinks: function(data) {
        ...
        data.links.each(function(link) {
            ...
            html += "<img title='Delete Link' style='cursor: pointer;' width='10px' height='10px' src='../img/delete.png' />&nbsp;&nbsp;";
            html += "<img title='Restart Link' style='cursor: pointer;margin-left: -4px' width='10px' height='10px' src='../img/arrow_refresh.png' /></div>";
    ...
    deletePackage: function(event) {
        ...
        new Request({
            method: 'get',
            url: '{{pathprefix}}/api/deletePackages/[' + this.id + "]",
            onSuccess: function() {
                this.ele.nix();
                indicateFinish();
            }.bind(this),
    ...
    restartPackage: function(event) {
        ...
        new Request({
            method: 'get',
            url: '{{pathprefix}}/api/restartPackage/' + this.id,
            onSuccess: function() {
                this.close();
                indicateSuccess();
            }.bind(this),
    ...
    movePackage: function(event) {
        ...
        new Request({
            method: 'get',
            url: '{{pathprefix}}/json/move_package/' + ((this.ui.type + 1) % 2) + "/" + this.id,
            onSuccess: function() {
                this.ele.nix();
                indicateFinish();
            }.bind(this),
    ...
    saveSort: function(ele, copy) {
        ...
        if (order.length > 0) {
            ...
            new Request.JSON({
                method: 'get',
                url: '{{pathprefix}}/json/link_order/' + order[0],
                onSuccess: indicateFinish,
                onFailure: indicateFail
            }).send();
        }
