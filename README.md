...
[![PyPI](https://img.shields.io/pypi/l/pyload.core.svg)](https://pypi.python.org/pypi/pyload.core)
...
To run the built-in `setup.py` you need at least
the package [setuptools](https://pypi.python.org/pypi/setuptools),
**version 17.1 or later**.

To install **pyLoad** using the built-in `setup.py` you need at least
...
for the updated requirements list.
All entries are mandatory _for the own scope_:

- `setup.txt` requirements are required by `setup.py` to **run itself**.
- `install.txt` requirements are required by `setup.py` to **install** the
pyLoad package.
- `test.txt` requirements are required by `setup.py` to **test** the
pyLoad package.
