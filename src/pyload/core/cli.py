...
manageable""")
    ...
    for prsr in (pg, sp_start, sp_stop, sp_restart, sp_status):
        profile_help = ''.join(
            autored("Profile name to use ("), autoyellow("`default`"), autored(" if missing)"))
    ...
    for prsr in (pg, sp_start, sp_restart):
        ...
        restore_help = ''.join(autored("Restore default login credentials "), autoyellow("`admin|pyload`"), autored(")"))
